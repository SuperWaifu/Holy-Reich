document.addEventListener('DOMContentLoaded', () => {
    ...
    musicItems.forEach((item, index) => {
        item.addEventListener('click', () => {
            ...
            updatePlayAllButtonsState(null); // mise à jour UI

            currentAudioPlayer = audioPlayers[index];
            updateMusicInfo(item.getAttribute('data-title'), item.getAttribute('data-artist'), item.getAttribute('data-cover'));
    ...
    function playNext(index) {
        if (!sequentialPlayActive || index < 0 || index >= sequentialPlayList.length) {
            updatePlayAllButtonsState(null);
            sequentialPlayActive = false;
            stopLogoSpin();
            return;
        }
	...
	// ---------- Gestion des boutons "Tout lire", "Aléatoire", "J'ai de la chance" ----------
    function updatePlayAllButtonsState(activeBtn = null) {
        document.querySelectorAll('.play-all-btn, .shuffle-btn, #lucky-btn').forEach(btn => btn.classList.remove('active'));
    ...
    document.querySelectorAll('.play-all-btn').forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            playMusicListSequentially(0, musicList, false);
            updatePlayAllButtonsState(button);
        });
    });
    ...
    document.querySelectorAll('.shuffle-btn').forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            playMusicListSequentially(0, shuffleArray(musicList), true);
            updatePlayAllButtonsState(button);
        });
    });
	
	// Lucky
    const luckyBtnElement = document.getElementById('lucky-btn');
    if (luckyBtnElement) {
        luckyBtnElement.addEventListener('click', () => {
            const allMusicItems = Array.from(document.querySelectorAll('.music-item'));
            if (!allMusicItems.length) return;

            const shuffledList = shuffleArray(allMusicItems);
            playMusicListSequentially(0, shuffledList, true);
            updatePlayAllButtonsState(luckyBtnElement);
        });
    }

    // Désactivation des boutons lorsqu'on clique ailleurs
    document.addEventListener('click', (e) => {
        if (!e.target.closest('.play-all-btn') && !e.target.closest('.shuffle-btn') && e.target.id !== 'lucky-btn' && !e.target.closest('.music-item')) {
            updatePlayAllButtonsState(null);
        }
        if (e.target.closest('.music-item')) updatePlayAllButtonsState(null);
    });
	...
	function showView(viewToShow) {
	  ...
	  views.forEach(v => {
		if (v === viewToShow) v.classList.add('active');
		else v.classList.remove('active');
	  });
	}

	lyricsBtn.addEventListener('click', () => {
		if (lyricsView.classList.contains('active')) showView(classicView);
		else showView(lyricsView);
	...
	queueBtn.addEventListener('click', () => {
	  ...
	  if (isActive) showView(classicView);
	  else {
		showView(queueView);
	...
	const lyricsDatabase = {
		...
		"A quel prix ?": "<strong>Couplet 1 :</strong><br>Tu marches encore droit, comme si rien n’avait brûlé,<br>Tu parles de demain, sans jamais regarder derrière.<br>Mais moi je vois les ombres que tu traînes à tes côtés,<br>Et j’entends le silence des cœurs que tu as laissés.<br><br><strong>Pré-refrain :</strong><br>On dit que le temps finit par tout effacer,<br>Mais le tien, dis-moi… comment peut-il t’épargner ?<br><br><strong>Refrain :</strong><br>Dis-moi… comment tu dors la nuit,<br>Quand tes mensonges pèsent plus lourd que ta vie ?<br>Quand chaque visage oublié revient dans tes rêves,<br>Comment fais-tu pour trouver le sommeil ?<br><br><strong>Couplet 2 :</strong><br>Tes mains sont propres, mais ton passé est taché,<br>Tu souris trop vite quand on parle du passé.<br>Peut-être qu’un jour, tes murs vont s’écrouler,<br>Et tu verras enfin tout ce que tu as brisé.<br><br><strong>Pré-refrain :</strong><br>Car les fantômes ne meurent pas dans le noir,<br>Ils attendent, ils frappent, au fond de ta mémoire.<br><br><strong>[Refrain]</strong><br><br><strong>Bridge :</strong><br>Un jour, faudra payer…<br>Et ce jour-là, j’aimerais te regarder.<br><br><strong>Refrain final :</strong><br>Dis-moi… comment tu dors la nuit,<br>Quand ton silence hurle plus fort que tes cris ?<br>Quand chaque larme que tu as volée se lève,<br>Comment fais-tu pour trouver le sommeil ?<br>",
		"La rivière emporte": "<strong>Couplet 1 :</strong><br>J’ai vu les nuages s’asseoir sur ma route,<br>Des ombres longues où le jour se dissout.<br>Les mains tremblantes cherchant la lumière,<br>Mais la pluie tombait toujours à l’envers.<br><br><strong>Pré-refrain :</strong><br>On dit qu’chaque pas te rapproche du matin,<br>Mais certains chemins n’ont plus de fin.<br><br><strong>Refrain :</strong><br>La rivière emporte ce qu’on ne retient pas,<br>Les secrets trop lourds, les rêves en éclats.<br>Et quand le courant devient plus fort que toi,<br>Tu fermes les yeux… et tu laisses aller tes pas.<br><br><strong>Couplet 2 :</strong><br>J’ai laissé mes bottes au bord du rivage,<br>Le vent d’ouest sifflait comme un vieil adage.<br>Il m’a parlé d’un monde sans orage,<br>Où les nuits sont courtes et les cœurs moins sages.<br><br><strong>Pré-refrain :</strong><br>Certains trouvent la paix au milieu des saisons,<br>Moi j’l’ai vue dans l’écho d’une chanson.<br><br><strong>[Refrain]</strong><br><br><strong>Pont :</strong><br>Si tu me cherches, regarde l’horizon,<br>Là où l’eau et le ciel font union.<br>Je n’ai pas fui, je suis juste parti,<br>Là où la rivière m’a promis la vie.<br><br><strong>Refrain final :</strong><br>La rivière emporte ce qu’on ne retient pas,<br>Les larmes, les chaînes, et les combats.<br>Et quand le courant devient plus fort que toi,<br>Tu fermes les yeux… et tu laisses aller tes pas.<br><br><strong>Outro :</strong><br>La rivière… m’a pris dans ses bras.",
